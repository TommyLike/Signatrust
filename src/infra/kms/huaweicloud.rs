...
impl HuaweiCloudKMS {
    ...
    pub fn new(config: &HashMap<String, Value>) -> Result<HuaweiCloudKMS> {
        Ok(HuaweiCloudKMS {
            kms_id: config.get("kms_id").unwrap_or(&Value::default()).to_string(),
            username: config.get("username").unwrap_or(&Value::default()).to_string(),
            password: config.get("password").unwrap_or(&Value::default()).to_string(),
            domain: config.get("domain").unwrap_or(&Value::default()).to_string(),
            project_name: config.get("project_name").unwrap_or(&Value::default()).to_string(),
            project_id: config.get("project_id").unwrap_or(&Value::default()).to_string(),
            endpoint: config.get("endpoint").unwrap_or(&Value::default()).to_string(),
    ...
    async fn auth_request(&self) -> Result<()> {
        ...
        let res = self.client.post(
            format!("https://iam.{}/v3/auth/tokens",
                    self.endpoint)).json(&auth_content).send().await?;
        if res.status() != StatusCode::CREATED {
            return Err(Error::KMSInvokeError(
                format!("failed to get huaweicloud token {} {:?}", res.status(), res.headers())))
        }
        self.auth_token_cache.lock().await.push_str(res.headers()[AUTH_HEADER].to_str()?);
        Ok(())
    }

    async fn do_request<T: Serialize + ?Sized>(&self, url: &str, json: &T)  -> Result<serde_json::Value> {
        self.auth_request().await?;
        let mut res = self.client
            .post(url).header(
            SIGN_HEADER, HeaderValue::from_str(self.auth_token_cache.lock().await.as_str())?)
...
impl KMSProvider for HuaweiCloudKMS {
    async fn encode(&self, content: String) -> Result<String> {
        ...
        let result = self.do_request(
            format!("https://kms.{}/v1.0/{}/kms/encrypt-data",
                    self.endpoint, self.project_id).as_str(), &request).await?;
    ...
    async fn decode(&self, content: String) -> Result<String> {
        ...
        let result = self.do_request(
            format!("https://kms.{}/v1.0/{}/kms/decrypt-data",
                    self.endpoint, self.project_id).as_str(), &request).await?;
